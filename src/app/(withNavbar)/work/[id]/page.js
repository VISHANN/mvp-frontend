      ...
      <div className={styles.gridWrap}>
        <section>
          ...
          <div className={styles.btnGroup}>
            <div>
              <ShelvesButton workId={params.id} />
            </div>
          </div>
        </section>
        <section>
          <div className={styles.work}>
            ...
            <div className={styles.banner}>
              ...
              {work.publication.first_published && (
                <span>
                  <span> • </span> First published in{" "}
                  {work.publication.first_published}
                </span>
              )}
              {work.publication.publishers[0] && (
                <span>
                  <span> • </span> Published by {work.publication.publishers[0]}
                </span>
              )}
            </div>
          </div>
...
async function getWorkMetadata(workId) {
  ...
  for (const e of data.authors) {
    const author = await fetch(
      `https://openlibrary.org${e.author.key}.json`
    ).then((res) => res.json());
